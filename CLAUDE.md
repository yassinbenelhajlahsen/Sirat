...
- **Sync:** The sync engine lives in `frontend/services/sync/`. It runs only when the user is signed in and online, pushing the 4 domains (`prayer_log`, `habits`, `habit_log`, `settings`) to `POST /api/sync` and applying the merged response locally. The `settings` domain carries the four user-facing notification preferences — `notif_map_v1`, `notif_sound_mode_v1`, `notif_window_map_v1`, `notif_window_offset_v1` (envelope keys `notifPrefs`/`notifSoundMode`/`notifWindowMap`/`notifWindowOffset`). Device-specific notification state (`notif_enabled_v1`, OS status, schedule/seen/daykey, city-display keys) and the mosque/prayer-times caches never sync. Each synced notif pref is LWW-stamped off its own granular change event (`NOTIF_MAP_CHANGED` etc.), NOT the broadcast `NOTIF_PREFS_UPDATED` — that event also fires on launch and master-toggle/permission changes, which would otherwise re-stamp all four every launch and let the local device always win. Settings stamping uses a sidecar key `sync:settings_meta_v1`; last sync time persists in `sync:last_synced_v1`. **Known limitation:** the `settings` domain merges each setting as a whole value under one LWW stamp. For the two *accumulating collections* (`quranBookmarks`, `ramadanTracker`) this means concurrent additions on two offline devices can lose entries (the lower-stamped device's whole list is discarded on merge) — acceptable for now; revisit with per-item `Cell`-keyed merge (like `habitLog`) if it bites. The tracker domains (prayer log, habits, habit log) already merge per-cell and are unaffected.
